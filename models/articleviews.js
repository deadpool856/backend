...
exports.getById = async function getById(id){
    let query = "SELECT * FROM articleviews WHERE ID = ?";
    let values = [id];
...
exports.getAll = async function getAll (page, limit, order) {
    let query = "SELECT * FROM articleviews ;";
...
exports.delete = async function del(id) {
    let query = "DELETE FROM articleviews WHERE ID = ?";
    let values = [id];
    let data = await db.run_query(query, values);
    return data;
}
...
exports.update = async function update(id, article) {
    let query = "UPDATE articleviews SET ? WHERE ID = ?";
